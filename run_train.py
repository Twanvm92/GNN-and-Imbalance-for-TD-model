...
# flags.DEFINE_integer('early_stopping',70, 'Tolerance for early stopping (# of epochs).')
...
placeholders = {
    ...
    'steps_per_epoch': tf.placeholder_with_default(0.0, shape=()), #用于学习率上 2020 9 30
    # 'epoch': tf.placeholder_with_default(0, shape=()), #用于学习率上 2020 7 20
    'd_lossweight': d_lossweight #加权系数 2020 7 22
}
...
for epoch in range(FLAGS.epochs):
    ...
    for start in range(0, len(train_y), FLAGS.batch_size):
        ...
        # feed_dict.update({placeholders['epoch']: epoch + 1})  # 2020 7 20 用于学习率
        feed_dict.update({placeholders['steps_per_epoch']: steps_per_epoch})  # 2020 9 30 用于学习率

        outs = sess.run([model.opt_op, model.loss, model.accuracy,model.learning_rate], feed_dict=feed_dict)
        train_loss = outs[1]
        train_acc = outs[2]
        print("learning_rate",outs[3])
    ...
    # train_acc /= len(train_y)
        ...
        if currentStep % FLAGS.evaluateEvery == 0:
            ...
            #       "val_acc=", "{:.5f}".format(val_acc), "test_f1=", "{:.5f}".format(test_f1),
            #       "time=", "{:.5f}".format(time.time() - t), "best_f1=", "{:.5f}".format(best_f1))
...
#
# print("Epoch:", '%04d' % (epoch + 1), "train_loss=", "{:.5f}".format(train_loss),
#                   "val_loss=", "{:.5f}".format(val_cost),"val_f1=", "{:.3f}".format(val_f1), "test_pre=", "{:.3f}".format(test_pre), "test_recall=",
#                   "{:.3f}".format(test_recall), "test_f1=", "{:.3f}".format(test_f1),
#                   "time=", "{:.5f}".format(time.time() - t), "best_f1=", "{:.3f}".format(best_f1))
# result = "Epoch:" + "%04d" % (epoch + 1) + " train_loss=" + "{:.5f} ".format(
#                 train_loss)  + " val_loss="+ "{:.5f}".format(val_cost)+" val_f1="+ "{:.3f}".format(val_f1)+ " test_pre=" + "{:.3f} ".format(
#                 test_pre) + " test_recall=" + "{:.3f} ".format(
#                 test_recall) + " test_f1=" + "{:.3f} ".format(
